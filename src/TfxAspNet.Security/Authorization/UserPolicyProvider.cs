...
namespace Xyz.TForce.AspNet.Security.Authorization
{
  ...
  internal class UserPolicyProvider : INamedPolicyProvider
  {

    public bool IsSupported(string policyName)
    {
      return policyName.StartsWith(PolicyTypes.User);
    }
    ...
    public AuthorizationPolicy GetPolicy(string policyName)
    {
      ...
      {
        _ = policyBuilder.AddRequirements(new UserRequirement(policyParts[1]));
      }

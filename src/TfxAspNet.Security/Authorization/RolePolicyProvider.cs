...
namespace Xyz.TForce.AspNet.Security.Authorization
{
  ...
  internal class RolePolicyProvider : INamedPolicyProvider
  {

    public bool IsSupported(string policyName)
    {
      return policyName.StartsWith(PolicyTypes.Role);
    }
    ...
    public AuthorizationPolicy GetPolicy(string policyName)
    {
      ...
      {
        _ = policyBuilder.AddRequirements(new RoleRequirement(policyParts[1]));
      }
